...
public class IBWLogic {
    ...
    static boolean checkIfExists(char c, char[] secretWordString, int currentIndex) {
        for (int i = 0; i < secretWordString.length; i++) {
            if (i != currentIndex && secretWordString[i] == c) {
                return true;
            }
        }
        return false;
    ...
    static void validateGuess(){
        ...
        for (int i = 0; i < MAX_COLS; i++) {
            ...
            if (currentChar == secretWordChar[i]) {
                ...
                count++;
            } else if (checkIfExists(currentChar, secretWordChar, i) && IBWGraphics.getCellColor(j,i)!=COLOR_YELLOW)       {
